...
export class AuthenticationService {
	...
	register(user: User) {
		return this.http.post(`${this.apiUrl}/api/v1/register`, user);
	}

	login(email: string, password: string) {

			return this.http.request('post', `${this.apiUrl}/api/v1/se-connecter`,{
			   ...
			   .pipe(map((user:any) => {
				...
				let data = {
					...
					if (user) {
						...
						localStorage.setItem('if_responsable', JSON.stringify(user.user.responsable));
	...
	logout(isheader = true) {
		...
		if (isheader) {
			this.toastrService.success("Vous vous êtes déconnecté avec succès.");
			   ...
			   localStorage.removeItem('if_responsable');
					setTimeout(() => {
