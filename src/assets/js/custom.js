...
function getDemo(placeId){
  //  console.log(placeId);

    let t = $(this);
            
    //let placeId =  placeId;
    let placesService = new google.maps.Geocoder();

    var request = {
      placeId: placeId
    };

    placesService.geocode(request, function(place, status) {

      if (status === google.maps.GeocoderStatus.OK) {
  //console.log(place[0].geometry.location.lng());
        
      $('.mapgoogle-autocomplete-form-custom').closest('.form-group').find('input[name="lng"]').attr('value', place[0].geometry.location.lng()).trigger('change');
      $('.mapgoogle-autocomplete-form-custom').closest('.form-group').find('input[name="lat"]').attr('value', place[0].geometry.location.lat()).trigger('change');
      $('.mapgoogle-autocomplete-form-custom').closest('.form-group').find('input[name="address"]').attr('value', place[0].formatted_address).trigger('change');
      //$('.mapgoogle-autocomplete-form-custom').closest('.form-group').find('input[name="address"]').attr('value', ).trigger('change');

      //this.departValue = place[0].formatted_address;
      $('.mapgoogle-autocomplete-custom').val(place[0].formatted_address);
        // Hide autocomplete results
      // results.style.display = 'none';
        $('#custom-output-autocomplete').css('display','none');
      }
    });
}
...
(function(jQuery) {
    ...
    jQuery(document).ready(function() {
        ...
        jQuery('.pac-item').on('click', function() {
            console.log('placeid');
            
            let t = $(this);
            
           let placeId =  t.data('placeid');
           let placesService = new google.maps.Geocoder();
